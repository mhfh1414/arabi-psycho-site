# -*- coding: utf-8 -*-
# dsm_suite.py — ملف واحد: DSM موسّع + دراسة حالة + واجهة HTML داخلية (Blueprint)
...
SYNONYMS = {
    "حزن": ["زعل","كابه","تعاسه","ضيقه","طفش","غم","نكد"],
    "انعدام المتعة": ["فقدان المتعه","ما استمتع","ما عاد يفرحني شي","فقدان الاهتمام"],
    "طاقة منخفضة": ["خمول","كسل","ارهاق","تعب","وهن","هبوط طاقه","نعاس"],
    ...
    "شهية زائدة": ["نهم","اكل كثير","اكل عاطفي"],
...
def result_view(form):
    ...
    details = score(symptoms, duration_days=duration, history=history)
...
def dsm_root():
    if request.method == "POST":
        return result_view(request.form)
    return form_view()
