# -*- coding: utf-8 -*-
# app.py — Arabi Psycho (v2.8.1 One-File Stable, Fixed)
...
<div class="card">
  ...
  <script>
    ...
    // تحسّب: بنبني جدول افتراضي تلقائي أول مرة حتى تتأكد أنها شغالة
    window.addEventListener('DOMContentLoaded', ()=>{{ if(!document.getElementById('checklist').innerHTML) buildChecklist(); }});
  </script>
</div>
"""
...
<div class="card">
  ...
  <form method="post" action="/case" oninput="persistCase()">
    <div class="grid">
      ...
      <div class="tile"><h3>اضطرابات النوم/الأكل/الانتباه</h3>
...
def build_recommendations(data: dict) -> Tuple[List[Tuple[str,str,int]], List[str], List[str]]:
    ...
    if c(data,"binge_eating","restrict_eating") >= 1:
        # FIX: كانت هنا فاصلة عربية تسبب SyntaxError — تم استبدالها بفاصلة إنجليزية صحيحة
