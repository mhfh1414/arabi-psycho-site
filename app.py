...
#
# مهم:
# - لا تعتبر أي شيء هنا تشخيص طبي أو وصفة علاج. لازم طبيب مختص.
# - كل التخزين محلي في المتصفح (localStorage) مش في السيرفر.
# ======================================================================
...
<div class="grid">
  ...
  <div class="tile" style="border:2px solid #000;">
    ...
    <p class="small">
      ...
      <br/>وش يُستخدم وليه؟ أهم الأعراض الجانبية؟ متى لازم دكتور فورًا؟
    </p>
...
def case():
    ...
    _ = request.form.get("notes", "").strip()
...
# ======================== /pharm ========================
# صفحة الأدوية النفسية: تثقيف فقط. ما فيه جرعات، ما فيه وصفة.
# فيها بحث + كروت أدوية + تحذير واضح.
...
<div class="card" style="border:2px solid #000;">
  ...
  <script>
    ...
    // بدون جرعات: فقط 'يُستخدم في ماذا' + 'أعراض جانبية' + 'تحذير'
    ...
    const MEDS = [
      ...
      },
      {
        ...
        warn:"جرعات أعلى تحتاج مراقبة نظم القلب لبعض الناس."
      },
      {
        ...
        warn:"تداخلات مع أدوية ثانية؛ لا تخلط عشوائي."
      },
      ...
      // SNRIs
      {
        ...
        warn:"إيقاف سريع = دوخة/شعور صدمات كهربائية. لازم نزول تدريجي."
      },
      {
        name:"ديدزفينلافاكسين (Desvenlafaxine)",
        klass:"SNRI",
        uses:"اكتئاب",
        sfx:"غثيان، تعرّق، خدران خفيف بالوجه/الأطراف",
        warn:"نفس مبدأ فينلافاكسين: لا توقف فجأة."
      },
      {
        ...
        warn:"أحيانًا يراجعون الكبد. يحتاج إشراف."
      },
      ...
      // مضادات اكتئاب غير تقليدية
      {
        ...
        uses:"اكتئاب، دعم الإقلاع عن التدخين، نقص دافعية/طاقة",
        sfx:"أرق، قلق، صداع",
        warn:"يرفع احتمال التشنجات عند الجرعات العالية أو مع أكل قليل جدًا."
      },
      {
        ...
        warn:"ينوّم بشدة عند كثير ناس، غالبًا يُؤخذ ليل تحت إشراف."
      },
      {
        name:"ترازودون (Trazodone)",
        klass:"مضاد اكتئاب يُستخدم كثير كمنوّم",
        uses:"أرق مرتبط باكتئاب/قلق",
        sfx:"نعاس، دوخة صباح، جفاف فم",
        warn:"نادر جدًا يسبب انتصاب مؤلم طويل؛ هذا طارئ طبي."
      },
      {
        name:"فورتييوكسيتين (Vortioxetine)",
        klass:"مضاد اكتئاب متعدد المستقبلات",
        uses:"اكتئاب مع ضعف التركيز/التفكير",
        sfx:"غثيان خفيف غالبًا",
        warn:"أي أفكار أذية أو انتحار لازم تبلّغ الطبيب فورًا."
      },
      ...
      // مثبتات مزاج / ثنائي القطب
      {
        ...
        uses:"ثنائي القطب (هوس/اكتئاب) ويقلل خطر الانتحار",
        sfx:"عطش، تبول متكرر، رعشة خفيفة باليد، زيادة وزن بسيطة",
        warn:"لازم تحاليل دم لمستوى الليثيوم. الجرعة الخطأ = تسمم خطير."
      },
      {
        name:"ديفالبروإكس / فالبروات (Valproate / Depakote)",
        klass:"مثبت مزاج/مضاد نوبات",
        uses:"هوس حاد، غضب وانفجارات انفعالية شديدة",
        sfx:"زيادة وزن، نعاس، غثيان",
        warn:"يراقَب الكبد والدم. خطير في الحمل بدون متابعة متخصصة."
      },
      {
        name:"لاموتريجين (Lamotrigine / لامكتال)",
        klass:"مثبت مزاج (قوي خاصة على اكتئاب ثنائي القطب)",
      ...
      },
      {
        ...
        sfx:"دوار، نعاس، غثيان",
        warn:"لازم متابعة تعداد الدم وإنزيمات الكبد. تداخلات أدوية قوية."
      },
      ...
      // مضادات الذهان (ذهان/فصام/هوس شديد)
      {
        ...
        uses:"ذهان، هوس، وأحيانًا قلق/أرق شديد بجرعات صغيرة بإشراف",
        sfx:"نعاس، زيادة وزن، شهية عالية",
        warn:"يسبب خمول قوي. لا تسوق لو أنت نعسان."
      },
      {
        ...
        warn:"لازم مراقبة سكر ودهون لأن الشهية تزيد بقوة."
      },
      {
        name:"ريسبيريدون (Risperidone / ريسبردال)",
        klass:"مضاد ذهان غير نمطي",
        uses:"فصام، هوس، اندفاع/عدوانية عند بعض الاضطرابات السلوكية",
        sfx:"تيبّس عضلي بسيط، نعاس، زيادة وزن متوسطة",
        warn:"بجرعات أعلى ممكن يرفع هرمون الحليب (حساسية بالصدر عند البعض)."
      },
      {
      ...
      },
      {
        ...
        warn:"يُؤخذ غالبًا مع الأكل، ويراقَب نظم القلب."
      },
      {
        ...
        warn:"غالبًا زيادة وزن أقل من بعض الأدوية الثانية لكن برضو يحتاج متابعة."
      },
      {
        ...
        warn:"لازم فحوص دم دورية (كرات الدم البيضاء). يُصرف فقط تحت طبيب متخصص."
      },
      {
        name:"هالوبيريدول (Haloperidol / هالدول)",
        klass:"مضاد ذهان قديم (تيبيكَل)",
        uses:"ذهان حاد، هياج شديد (غالبًا طوارئ)",
        sfx:"تيبّس/تشنجات عضلية حادة خصوصًا بجرعات عالية",
        warn:"عادة يُعطى في بيئة طبية مراقبة."
      },
      ...
      // قلق / مهدئات / نوم / أعصاب
      {
        ...
        sfx:"نعاس قوي، بطء تفكير، قابلية تعلق دوائي",
        warn:"إدماني. إيقاف فجأة ممكن يسبب انسحاب خطير/نوبات صرع."
      },
      {
        name:"ألبرازولام (Alprazolam / زاناكس)",
        klass:"بنزوديازبين قصير المفعول",
        uses:"نوبات هلع حادة، قلق شديد قصير المدى",
        sfx:"نعاس، فقدان ذاكرة قصير المدى، تعلق سريع",
      ...
      },
      {
        ...
        warn:"نفس العائلة الإدمانية. لا توقف بدون نزول تدريجي طبي."
      },
      {
        ...
        warn:"خطر مع القيادة. قابل للإدمان مع الاستخدام المزمن."
      },
      {
        ...
        warn:"يأخذ أسابيع ليشتغل. مو إسعافي للهلع."
      },
      {
        ...
        sfx:"نعاس، جفاف فم",
        warn:"يسبب نعاس قوي عند بعض الناس. جرّب قبل السواقة."
      },
      {
        name:"بروبرانولول (Propranolol)",
        klass:"حاصر بيتا",
        uses:"قلق الأداء الجسدي (رجفة، خفقان قبل مواجهة/عرض)",
        sfx:"تعب، برودة أطراف، دوار خفيف",
        warn:"مو لكل أحد (مثل الربو). لازم تقييم طبي."
      },
      {
        name:"بريجابالين (Pregabalin / ليريكا)",
        klass:"مهدئ أعصاب/قلق وألم عصبي",
        uses:"قلق عام (في بعض الدول)، ألم أعصاب، ألم مزمن",
        sfx:"دوخة، نعاس، إحساس 'هاي' عند بعض الناس",
        warn:"قابل لسوء الاستخدام/الإدمان. مراقب في دول كثيرة."
      },
      {
        name:"جابابنتين (Gabapentin)",
        klass:"لعلاج ألم الأعصاب وأحيانًا قلق/أرق خفيف بإشراف",
        uses:"ألم أعصاب مزمن، قلق خفيف، صعوبة نوم",
        sfx:"دوخة، ترنح، نعاس",
        warn:"صار يُساء استخدامه عند البعض. مو مسكّن حر آمن."
      },
      {
        name:"زولبيديم (Zolpidem / أمبيان)",
        klass:"منوّم قصير المدى",
        uses:"أرق حاد قصير المدى",
        sfx:"نعاس، وأحيانًا سلوكيات نوم غريبة (مشي/أكل)",
        warn:"لا يخلط مع كحول. مو حل مزمن يومي بدون تقييم."
      },
      ...
      },
      {
        ...
        uses:"ADHD، وأحيانًا الأكل بنهم بإشراف",
      ...
      // إدمان / تعاطي
      {
        ...
        uses:"تقليل الرغبة في شرب الكحول وتقليل 'النشوة' من الأفيونات",
        sfx:"غثيان، صداع، تعب",
        warn:"لا يُؤخذ لو فيه أفيون نشط بالجسم بدون بروتوكول، لأنه مم
