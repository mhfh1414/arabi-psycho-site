app.py — ArabiPsycho (v2.1)

نسخة مُحسّنة: تثبيت الواجهة، حفظ تلقائي في المتصفح، تحسين المشاركة، API بسيطة، رؤوس أمان، وعداد زوار آمن بالكتابة الذرّية

ملاحظة: ملف واحد فقط كما تحب يا أبو فارس — جاهز لرفعُه مباشرة على Render

import os, urllib.parse, json, tempfile, io from datetime import datetime from flask import Flask, request, redirect, send_file, jsonify, make_response

app = Flask(name)

================= إعدادات عامة =================

BRAND = os.environ.get("BRAND_NAME", "عربي سايكو") LOGO  = os.environ.get("LOGO_URL", "https://upload.wikimedia.org/wikipedia/commons/3/36/Emoji_u1f985.svg") TG_URL = os.environ.get("TELEGRAM_URL", "https://t.me/arabipsycho") WA_URL = os.environ.get("WHATSAPP_URL", "https://wa.me/966530565696?text=%D8%B9%D8%B1%D8%A8%D9%8A%20%D8%B3%D8%A7%D9%8A%D9%83%D9%88")

روابط التحويل حسب نوع الموعد

PSYCHO_WA = os.environ.get("PSYCHOLOGIST_WA", "https://wa.me/966530565696") PSYCH_WA  = os.environ.get("PSYCHIATRIST_WA", "https://wa.me/966530565696") SOCIAL_WA = os.environ.get("SOCIAL_WORKER_WA", "https://wa.me/966530565696")

================= عدّاد الزوّار (كتابة ذرّية) =================

COUNTER_FILE = "visitors.json"

def atomic_write(path: str, data: dict): tmp_fd, tmp_path = tempfile.mkstemp(prefix="vis", suffix=".json") try: with os.fdopen(tmp_fd, "w", encoding="utf-8") as f: json.dump(data, f, ensure_ascii=False) f.flush() os.fsync(f.fileno()) os.replace(tmp_path, path)  # ذرّي على نفس القسم finally: try: if os.path.exists(tmp_path): os.remove(tmp_path) except Exception: pass

def _load_count() -> int: try: with open(COUNTER_FILE, "r", encoding="utf-8") as f: return int(json.load(f).get("count", 0)) except Exception: return 0

def _save_count(n: int): try: _atomic_write(COUNTER_FILE, {"count": int(n)}) except Exception: pass

def bump_visitors() -> int: n = _load_count() + 1 _save_count(n) return n

================= إطار الصفحات =================

CACHE_BUST = os.environ.get("CACHE_BUST", datetime.utcnow().strftime("%Y%m%d%H%M%S"))

def shell(title: str, content: str, visitors: int | None = None) -> str: visitors_html = f"<div class='small' style='margin-top:12px'>👀 عدد الزوّار: <b>{visitors}</b></div>" if visitors is not None else "" return f"""<!doctype html><html lang="ar" dir="rtl"><head> <meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
...
@app.get("/") def home(): visitors = bump_visitors() content = f""" <div class="card" style="margin-bottom:14px"> <h1>مرحبًا بك في {BRAND}</h1> <div class="small">مساحتك الهادئة لفهم الأعراض وبناء خطة عملية محترمة لخصوصيتك.</div> </div> <div class="grid"> <div class="tile"><h3>📝 دراسة الحالة</h3><p class="small">قسّم الأعراض بدقة؛ ترتبط بالـ CBT وبرنامج الإدمان والحجز.</p><a class="btn gold" href="/case">ابدأ الآن</a></div> <div class="tile"><h3>📘 مرجع DSM</h3><p class="small">ملخّص منظّم للمحاور الكبرى.</p><a class="btn alt" href="/dsm">فتح DSM</a></div> <div class="tile"><h3>🧠 CBT</h3><p class="small">15 خطة علمية + مولّد جدول 7/10/14 يوم (دمج خطتين) + حفظ تلقائي.</p><a class="btn" href="/cbt">افتح CBT</a></div> <div class="tile"><h3>🚭 برنامج الإدمان</h3><p class="small">Detox → Rehab → Aftercare → منع الانتكاس.</p><a class="btn" href="/addiction">افتح الإدمان</a></div> <div class="tile"><h3>📅 احجز موعدًا</h3><p class="small">الأخصائي النفسي / الطبيب النفسي / الأخصائي الاجتماعي.</p><a class="btn gold" href="/book">نموذج الحجز</a></div> <div class="tile"><h3>تواصل سريع</h3><a class="btn tg" href="{TG_URL}" target="_blank" rel="noopener">تيليجرام</a> <a class="btn wa" href="{WA_URL}" target="_blank" rel="noopener">واتساب</a></div> </div> """ return shell("الرئيسية — عربي سايكو", content, visitors)
...
@app.get("/dsm")
def dsm():
    return shell("DSM — مرجع", DSM_HTML, _load_count())================= CBT (خطط + جدول + حفظ تلقائي + مشاركة مُحسّنة) =================

CBT_HTML = f"""

<div class=\"card\">
  <h1>🧠 العلاج المعرفي السلوكي (CBT)</h1>
  <p class=\"small\">اختر خطة/خطة+خطة ثم أنشئ جدول أيام 7/10/14 تلقائيًا مع مربعات إنجاز وتنزيل/طباعة/مشاركة. <b>الاختيارات تُحفظ تلقائيًا في جهازك</b>.</p>  <h2>خطط جاهزة (15 خطة)</h2>
  <div class=\"grid\"><div class=\"tile\"><h3>BA — تنشيط سلوكي</h3><ol>
...
<div class=\"tile\"><h3>SS — مهارات اجتماعية</h3><ol>
  ...
  </div>  <h2 style=\"margin-top:18px\">📅 مولّد جدول الأيام (يدعم دمج خطتين)</h2>
  <div class=\"tile\">
    <div class=\"row\">
      <label>الخطة A:
    ...
    function fillOptions(){
      ...
      for(const k in PLANS){
        const o=document.createElement('option'); o.value=k; o.textContent=PLANS[k].title; selectB.appendChild(o);
      }
    }
    fillOptions();
    // استرجاع آخر اختيار
    ...
    function saveChecklist(){
      ...
      const progress = [];
      rows.forEach((tr, idx)=>{
        const day=idx+1; const done=[...tr.querySelectorAll('input[type=checkbox]')].map(ch=>ch.checked);
...
@app.get("/cbt")
def cbt():
    return shell("CBT — خطط وتمارين", CBT_HTML, _load_count())================= برنامج الإدمان =================
...
@app.get("/addiction")
def addiction():
    return shell("علاج الإدمان", ADDICTION_HTML, _load_count())================= نموذج الحجز =================
...
<div class=\"card\">
  ...
  <script>
    function validateBook(){
      const phone=document.querySelector('[name=\\x22phone\\x22]');
...
def book():
    ...
    return redirect(wa_link, code=302)================= دراسة الحالة =================

def c(data,*keys):  # count true (وجود في POST) return sum(1 for k in keys if data.get(k) is not None)

FORM_HTML = """

<div class=\"card\">
  <h1>📝 دراسة الحالة</h1>
  <div class=\"small\">قسّم الأعراض بدقة؛ ستظهر ترشيحات أولية وروابط لأدوات CBT وبرنامج الإدمان والحجز. <b>اختياراتك تُحفظ تلقائيًا</b>.</div>  <form method=\"post\" action=\"/case\" oninput=\"persistCase()\">
    <div class=\"grid\">
      ...
      </div><div class=\"tile\"><h3>اكتئاب — أعراض إضافية</h3>
...
<button class=\"btn gold\" type=\"submit\">عرض الترشيحات</button>
    ...
    function persistCase(){
      const form=document.querySelector('form[action=\\x22/case\\x22]');
      const data={};
    ...
    (function restore(){
      try{ const data=JSON.parse(localStorage.getItem(KEY)||'{}');
        Object.keys(data).forEach(k=>{
          const el=document.querySelector('[name=\\x22'+k+'\\x22]');
...
"""def build_recommendations(data): picks, go_cbt, go_add = [], [], []
...
def render_results(picks, go_cbt, go_add, notes): items_li = "".join([f"<li><b>{t}</b> — {w} <span class='small'>(درجة: {s:.0f})</span></li>" for (t,w,s) in picks]) or "<li>لا توجد مؤشرات كافية.</li>" cbt_badges = "".join([f"<span class='badge2'>🔧 {x}</span>" for x in go_cbt]) add_badge  = "<span class='badge2'>🚭 برنامج الإدمان مُقترح</span>" if go_add else "" header = f""" <div class='header-result'> <img src='{LOGO}' alt='logo' onerror="this.style.display='none'"> <div><div style='font-weight:900;font-size:22px'>{BRAND}</div> <div class='small'>نتيجة دراسة الحالة — تلخيص أولي جاهز للطباعة والمشاركة</div></div> </div>""" summary = f""" <div class='summary-cards'> <div class='scard'><b>الترشيحات</b><br/><span class='small'>{len(picks)} نتيجة</span></div> <div class='scard'><b>CBT المقترح</b><br/>{(cbt_badges or "<span class='small'>—</span>")}</div> <div class='scard'><b>الإدمان</b><br/>{(add_badge or "<span class='small'>—</span>")}</div> </div>""" note_html = f"<div class='tile' style='margin-top:10px'><b>ملاحظاتك:</b><br/>{notes}</div>" if notes else "" actions = f""" <div class='row screen-only' style='margin-top:12px'> <button class='btn alt' onclick='window.print()'>🖨️ طباعة</button> <button class='btn' onclick='saveJSON()'>💾 تنزيل JSON</button> <a class='btn wa' id='share-wa' target='_blank' rel='noopener'>🟢 مشاركة واتساب</a> <a class='btn tg' id='share-tg' target='_blank' rel='noopener'>✈️ مشاركة تيليجرام</a> <a class='btn gold' href='/book'>📅 حجز سريع</a> <a class='btn' href='/cbt'>🧠 فتح CBT</a> </div> <div class='print-only small' style='margin-top:8px'>تم إنشاء هذا الملخّص بواسطة <b>{BRAND}</b> — {TG_URL}</div> <script> function buildShareText(){ const items=[...document.querySelectorAll('#diag-items li')].map(li=>' - '+li.innerText).join('\n'); const notes={json.dumps(notes or "")!r}; let msg='نتيجة دراسة الحالة — {BRAND}\n\n'+items; if(notes) msg+='\n\nملاحظات: '+notes; msg += '\n' + location.origin + '/case'; return msg; } function saveJSON(){ const data={items:[...document.querySelectorAll('#diag-items li')].map(li=>li.innerText), cbt:[...document.querySelectorAll('.badge2')].map(b=>b.innerText), notes:{json.dumps(notes or "")!r}, created_at:new Date().toISOString(), build: window.BUILD}; const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:'application/json'})); a.download='case_result.json'; a.click(); URL.revokeObjectURL(a.href); } const text=encodeURIComponent(buildShareText()); document.getElementById('share-wa').href='{WA_URL.split("?")[0]}'+'?text='+text; document.getElementById('share-tg').href='https://t.me/share/url?url='+encodeURIComponent(location.origin+'/case')+'&text='+text; </script>""" return f""" <div class='card'> {header}{summary} <h2 style='margin-top:12px'>📌 الترشيحات</h2> <ol id='diag-items' style='line-height:1.95; padding-inline-start: 20px'>{items_li}</ol> <h3>🔧 أدوات CBT المقترحة</h3> <div>{cbt_badges or "<span class='small'>لا توجد أدوات محددة</span>"}</div> <h3 style='margin-top:10px'>🚭 الإدمان</h3> <div>{add_badge or "<span class='small'>لا مؤشرات</span>"}</div> {note_html}{actions} </div>"""

@app.route("/case", methods=["GET","POST"]) def case(): if request.method == "GET": return shell("دراسة الحالة", FORM_HTML, _load_count()) data  = {k: v for k, v in request.form.items()} picks, go_cbt, go_add = build_recommendations(data) notes = (request.form.get("notes") or "").strip() return shell("نتيجة الترشيح", render_results(picks, go_cbt, go_add, notes), _load_count())

================= تواصل =================

@app.get("/contact") def contact(): html = f""" <div class='card'> <h1>📞 التواصل</h1> <div class='grid'> <div class='tile'><h3>قنوات عامة</h3> <a class='btn tg' href='{TG_URL}' target='_blank' rel='noopener'>تيليجرام عربي سايكو</a> <a class='btn wa' href='{WA_URL}' target='_blank' rel='noopener'>واتساب</a> </div> <div class='tile'><h3>حجز سريع</h3><a class='btn gold' href='/book'>📅 افتح نموذج الحجز</a></div> </div> </div>""" return shell("التواصل", html, _load_count())

================= API بسيطة لتكامل خارجي =================

@app.get("/api/health") def api_health(): return jsonify({"ok": True, "brand": BRAND, "build": CACHE_BUST}), 200

@app.get("/api/plans") def api_plans(): plans = { "ba": {"title": "BA — تنشيط سلوكي"}, "thought_record": {"title": "TR — سجل أفكار"}, "sleep_hygiene": {"title": "SH — نظافة النوم"}, "interoceptive_exposure": {"title": "IE — تعرّض داخلي"}, "graded_exposure": {"title": "GE — تعرّض تدرّجي"}, "ocd_erp": {"title": "ERP — وسواس قهري"}, "ptsd_grounding": {"title": "PTSD — تأريض/تنظيم"}, "problem_solving": {"title": "PS — حلّ المشكلات"}, "worry_time": {"title": "WT — وقت القلق"}, "mindfulness": {"title": "MB — يقظة ذهنية"}, "behavioral_experiments": {"title": "BE — تجارب سلوكية"}, "safety_behaviors": {"title": "SA — إيقاف سلوكيات آمنة"}, "bipolar_routine": {"title": "IPSRT — روتين ثنائي القطب"}, "relapse_prevention": {"title": "RP — منع الانتكاس (إدمان)"}, "social_skills": {"title": "SS — مهارات اجتماعية"}, } return jsonify({"brand": BRAND, "plans": plans, "build": CACHE_BUST})

================= صحة + 404 =================

@app.get("/health") def health(): return {"status":"ok","brand":BRAND,"build":CACHE_BUST}, 200

@app.errorhandler(404) def not_found(_): return shell("غير موجود", "<div class='card'><h1>الصفحة غير موجودة</h1><p class='small'>تفضل بالعودة للرئيسية.</p><a class='btn' href='/'>العودة</a></div>", _load_count()), 404

================= رؤوس أمان أساسية =================

@app.after_request def add_headers(resp): csp = "default-src 'self' 'unsafe-inline' data: blob: https://t.me https://wa.me https://api.whatsapp.com; img-src 'self' data: blob: *; connect-src 'self';" resp.headers['Content-Security-Policy'] = csp resp.headers['X-Content-Type-Options'] = 'nosniff' resp.headers['Referrer-Policy'] = 'strict-origin-when-cross-origin' resp.headers['Permissions-Policy'] = 'geolocation=()'  # لا نحتاج موقع return resp
