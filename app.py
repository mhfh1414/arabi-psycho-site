...
<div class="card">
  ...
  <script>
    ...
    function calcScore(){
      ...
      const selects = document.querySelectorAll('#quizBox select[data-q]');
      let total = 0;
      selects.forEach(s => total += parseInt(s.value||'0',10));

      let resultText = "—";
      for(const r of test.ranges){
        if(total <= r.max){ resultText = r.text; break; }
      }

      const resultBox = document.getElementById('resultBox');
      resultBox.style.display='block';
      resultBox.innerHTML = `
        <h3>🔹 النتيجة: ${total} من ${selects.length*3}</h3>
        <div class="note">${resultText}</div>
        <div class="small" style="margin-top:8px">
          نصيحة: شوف خطة CBT مناسبة (من نفس الصفحة الرئيسية) 👇
        </div>
...
@app.get("/tests")
def tests():
    return shell("اختبارات نفسية وشخصية — " + BRAND, TESTS_PAGE_HTML, "tests")
